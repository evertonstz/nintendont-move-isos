...
def copyfile(source, dest):
    t = threading.Thread(name='copying', target=shutil.copyfile, args=(source, dest))
    t.start()

    while t.is_alive():
        if os.path.exists(dest) and os.path.getsize(source) != os.path.getsize(dest):
            print( str(int((float(os.path.getsize(dest))/float(os.path.getsize(source))) * 100))+"%", end="")
            print('\r', end='')
            sleep(1)
    ...
    for file in files:
        if file.endswith(".iso"):
            ...
            if "\n"+gameid+" = " in wiitdb:
                ...
                gamename = wiitdb.split(gameid+" = ")[1].split("\n")[0].replace(":","-")
                newgamepath = os.path.join(dir,"games",gamename+" ["+gameid+"]")
...
print("Total ISOs copied: " + str(total_games), "Total ISOs ignored: "+ str(ignored_games))